...
const updateStory = async (req, res) => {
    try {
        ...
        const data = await ourstory_model.findByIdAndUpdate({ _id: id }, { heading, description, subheading1, content1, subheading2, content2, subheading3, content3, subheading4, content4, title, keyword, meta_description, image1: image1 ? image1 : currentData.image1, image2: image2 ? image2 : currentData.image2, image3: image3 ? image3 : currentData.image3, icon1: icon1 ? icon1 : currentData.icon1, icon2: icon2 ? icon2 : currentData.icon2, icon3: icon3 ? icon3 : currentData.icon3, icon4: icon4 ? icon4 : currentData.icon4 })

      

        if (!data) {
            [image1, image2, image3, icon1, icon2, icon3, icon4].forEach((image) => {
                if (image) {
